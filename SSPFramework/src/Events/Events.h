...
// The types of Mouse Buttons awailable
enum class Button
{
...
public:
    // Static variables representing the current event state:
    // The type of Event currently in process
    inline static EventType type = EventType::None;
    // The Key the user has last pressed or released (Note: Use the Input class for checking which Key you Pressed. This uses the old classic non buffered Event way.)
    inline static KeyCode keyCode;
    // The Mouse Button the user has last pressed or released (Note: Use the Input class for checking which Button you Pressed. This uses the old classic non buffered Event way.)
    inline static Button button;
    // The relative mouse position in screen space
    inline static Math::vec2 mousePosition{ 0, 0 };
    // The window position from the top left corner
    inline static Math::vec2 windowPosition{ 0, 0 };
    ...
    inline static float mouseScrollDelta;
    // The amount the mouse moved, positiv is (x) right and (y) up, negative is (x) left and (y) down
    inline static Math::vec2 mouseDelta;

private:
    // Restore the event state to its initial state
    static void Restore()
    {
        type = EventType::None;
    }

    // Dispatch an event of the specified type
    static void Dispatch(EventType e)
    {
        type = e;
    }
};